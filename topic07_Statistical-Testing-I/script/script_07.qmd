...
t_obs_row = pd.DataFrame(
    {
        "x": [t_obs],
        "y": [chi2.pdf(t_obs, df=df_val)]
    }
)

data_df_area = pd.concat(
    [t_obs_row, data_df_area]
    ).sort_values(by="x")
...
## create the table for the upper alternative
...
## create the table for the lower alternative
t_obs_low = chi2.ppf(chi2.sf(t_obs, df=df_val), df=df_val)

data_df_lower = data_df.loc[
    data_df["x"] <= t_obs_low
].copy()
...
new_row = pd.DataFrame(
    {
        "x": [t_obs_low],
        "y": [chi2.pdf(t_obs_low, df=df_val)],
    }
)
