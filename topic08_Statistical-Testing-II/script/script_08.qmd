...
We have already discussed how to test specific hypotheses, for instance considering the association between a genetic marker and growth rate in the yeast dataset (See Chapter @sec-resampling-chp). However, in the era of big data, we often do not restrict ourselves to testing just one single hypothesis. Molecular biologists can nowadays measure RNA abundance of all genes of a cell population. So what if we test the association of the RNA abundance of all ~8,000 yeast genes with every single genetic markers? For 1,000 genetic markers, this means we will have to do more than 8 million tests! 
...
#| echo: false
#| fig-align: center
#| fig-cap: Two-sided $P$-value for the coin tossing example. The two-sided $P$-value equals to the sum of the probabilities (total red area) under the null hypothesis of the realizations equal or more extreme than the observed one (vertical line).
...
mannwhitneyu(
    x=dt[dt['genotype'] == 'Lab strain']['growth_rate'].values, 
    y=dt[dt['genotype'] == 'Wild isolate']['growth_rate'].values, 
...
(
    ...
    + geom_boxplot()
    + coord_flip()
    + scale_y_log10() # Log transform the X-axis to handle the wide range due to the outlier
    + mytheme
    + theme(figure_size = (7, 4))
    + labs(y="Group", x="Value (Log Scale)") # Add clear labels
)
```
...
(ggplot(anscombe_df, aes('x', 'y')) + 
geom_point(size=3) + theme(text=element_text()) + 
labs(x= "X", y="Y") +
  mytheme+ theme(figure_size = (7, 4)))
```
...
(ggplot(dt_tan, aes(x='x', y='tan_x')) + geom_point() + mytheme + theme(figure_size = (7, 4)))
```
...
will achieve this because by definition `cdf(ppf(0.995))` is 0.995 and by symmetry `ppf(1-cdf(0.995))` is 1 - 0.995.  As a consequence, we have that:
