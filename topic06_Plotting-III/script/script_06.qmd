...
# Graphically Supported Hypotheses {#sec-graph-supp-hyp}
...
y = norm.rvs(loc=2 * z, scale=1, size=n)
...
```{python}
#| warning: false
df_common_cause.groupby("z").apply(
...
df_admissions = pd.read_csv(
    "../../extdata/UCBAdmissions.csv",
    index_col=0
)

df_admissions["is_admitted"] = np.where(
    df_admissions["Admit"] == "Admitted",
    1,
    0
)
...
(
    ...
    .apply(
        lambda g: (g["is_admitted"] * g["Freq"]).sum() / g["Freq"].sum()
    )
...
stats = (
    ...
    .assign(
        women_minus_men=lambda x: x["Female"] - x["Male"]
...
```{python}
df_admissions["is_female"] = np.where(
    df_admissions["Gender"] == "Female",
    1,
    0
)
...
(
    ggplot(
        selectivity_versus_percentage_women,
        aes("selectivity_rate", "percent_women_applicants", label = "Dept"))
        + geom_text()
        + labs(x="Major Selectivity", y="Percent Women Applicants")
        + theme(figure_size = (6, 4)
    )
)
```
...
#############
x_conseq = norm.rvs(0, 1, n)
y_conseq = norm.rvs(0, 1, n)
...
A set of color palettes you can use with `plotnine`:

![](../../assets/img/palettes.png){fig-align="center" width=70%}


We use qualitative palettes for categorical variables to separate items into distinct groups. In plotnine you can use `scale_fill_brewer` to create qualitative palettes:
```{python}
#| echo: true
#| eval: false
+ p9.scale_fill_brewer(type = "qual", palette = {palette of choice}) 
```
...
#| eval: true
num_colors = 9 
df_swatch_cat = pd.DataFrame(
    {
        "category": [f"Cat {i}" for i in range(num_colors)],
        "x": np.arange(num_colors),
    }
)

(
    ggplot(df_swatch_cat, aes(x="x", y=0, fill="category")) 
    + geom_tile(color="gray", size=0.5) 
    # Use scale_fill_brewer, specifying type="qual" and a qualitative palette name
    + scale_fill_brewer(type="qual", palette="Set1") 
    + theme_void()
    + theme(legend_position="none", figure_size = (9, 1))
)
```


Generally, we use sequential palettes for continuous variables to show quantitative differences. In plotnine you can use `scale_fill_distiller` to create sequential palettes :

```{python}
#| echo: true
#| eval: false
+ p9.scale_fill_distiller(type = "sequential", palette = {palette of choice})
```
...
#| echo: false
#| eval: true
df_swatch = pd.DataFrame({
    "x": np.arange(num_colors),
    "value": np.arange(num_colors),
    "value_centered": np.arange(- (num_colors // 2), (num_colors // 2) + 1) 
})

(
    ggplot(df_swatch, aes(x="x", y=0, fill="value")) 
    + geom_tile(color="gray", size=0.5)
    + scale_fill_distiller(type="sequential", palette="Greens")
    + theme_void()
    + theme(legend_position="none", figure_size = (9, 1))
)
```
...
We use diverging palettes for numeric variables that have a meaningful central value or breakout point (e.g. 0). In plotnine you can use `scale_fill_distiller` to create diverging palettes 

```{python}
#| echo: true
#| eval: false
+ p9.scale_fill_distiller(type = "diverging", palette = {palette of choice})
```
...
#| eval: true
(
    ggplot(df_swatch, aes(x="x", y=0, fill="value_centered"))
    + geom_tile(color="gray", size=0.5)
    + scale_fill_distiller(type="diverging", palette="RdBu")
    + theme_void()
    + theme(legend_position="none", figure_size = (9, 1))
)
```
...
(
    ...
    + geom_point(size=3, color = "#FFFFCC")
    + geom_smooth(method = "lm", se=False, color = "black", size = 0.5)
    + mytheme + theme(figure_size=(3, 2))
)
```
...
(
    ggplot(
        flightsLAX.head(2000),
        aes("DISTANCE", "AIR_TIME")
    )
    + geom_point(alpha=0.2)
    + geom_smooth(method="lm", color= "blue")
    + labs(
        title = "The further away an airport, the longer the flight",
        x = "Distance",
        y = "Flight duration"
    )
    + mytheme 
...
df_small = pd.DataFrame(
    {
        "Year": [2011, 2012, 2013],
        "Southwest_Border_Apprehensions": [165244,170223,192298]
    }
)
...
However, the plot is not genuinely 3D leading to confusion: We cannot really tell where the purple ribbon intersects the red one.
