...
```python
# BAD: This will give different results
# each time you run it
# If you run this cell twice, x becomes 12, then 13, etc.
x = 10
x = x + 1  
print(x)
```
...
# No matter how many times you run this, y is always 11
x = 10
y = x + 1  
print(y)
```
...
```{python}
# Code to compute solution to a quadratic equation
...
A **list** (`list`) is an ordered, mutable collection of items, which can be of different types. You access elements using their zero-based index.
...
Note, that the limits are specified as *intervals* and the third column won't be included.
...
### Lists And Arrays: References And Copies

List and array variables are just references. Always copy lists and arrays when reassigning! Compare:
...
url = (
    "https://raw.githubusercontent.com/rafalab/dslabs/"
...
```{python}
murder_rate = (
    murders["total"].values /
    murders["population"].values * 100_000
)
```
...
We can then use this boolean array inside square brackets `[]` to filter any other array of the same size, or even the whole dataframe, keeping only the rows where the condition is `True`.
...
```python
# Note the parentheses around each required condition
