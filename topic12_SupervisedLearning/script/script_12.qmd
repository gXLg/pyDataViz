...
display(Markdown(basic_df.to_markdown(index=False)))
```
...
n = 10
basic_df = pd.DataFrame({
    "outcome": [rf"$y_{{{i}}}$" for i in range(1, n + 1)],
    "feature_1": [rf"$x_{{{i},1}}$" for i in range(1, n + 1)],
    "feature_2": [rf"$x_{{{i},2}}$" for i in range(1, n + 1)],
    "feature_3": [rf"$x_{{{i},3}}$" for i in range(1, n + 1)],
    "feature_4": [rf"$x_{{{i},4}}$" for i in range(1, n + 1)],
    "feature_5": [rf"$x_{{{i},5}}$" for i in range(1, n + 1)],
})
...
As an example, we consider a polynomial curve fitting as a regression task with a dataset of $n =10$ points shown in @fig-poly1, each comprising an observation of the input variable $x$ along with the corresponding outcome variable $y$. In this example, the data is generated from the function $$f(x)=\sin(2\pi x)$$ (shown in blue) with added random noise from a normal distribution.

```{python}
#| echo: false
#| label: fig-poly1
#| fig-cap: 'Data set of $N =10$ points with a blue curve showing the function $\sin(2\pi x)$ used to generate the data.'
#| fig-width: 3
#| fig-height: 3
...
N = 9
x = np.linspace(0, 1, N)
y = np.sin(2 * np.pi * x) + rng.normal(0, 0.2, size=N)
poly_df = pd.DataFrame({"x": x, "y": y})

x_real = np.linspace(0, 1, 1000)
real_df = pd.DataFrame({"x_real": x_real, "y_real": np.sin(2 * np.pi * x_real)})

(
    ggplot(poly_df, aes("x", "y"))
    + geom_point()
    + geom_line(real_df, aes("x_real", "y_real"), color="blue")
    + my_theme
    + theme(figure_size=(4, 3))
)
```

Assuming that we do not know the function plotted in the blue curve, the goal is to predict the value of $y$ for some new arbitrary value of $x$. For this, we want to exploit the given dataset of $n$ data points in order to make predictions of the value $y$ of the outcome variable for some new value $x$, which involves implicitly trying to discover the underlying function $f(x)=\sin(2\pi x)$. This is a difficult problem considering that we have to generalize from a finite dataset of only $n$ points. Furthermore, the given data points include noise, and so for a given $x$, there is a certain uncertainty regarding the correct value for $y$.
...
Here, $m$ is the order of the polynomial function and $x^j$ denotes $x$ raised to the power of $j$. The polynomial coefficients $\beta_0,...,\beta_m$ are denoted by the vector $\boldsymbol{\beta}$. Note that, the polynomial function $y(x, \boldsymbol{\beta})$ is a nonlinear function of $x$ but it is a linear function of the coefficients $\boldsymbol{\beta}$. Hence, a linear regression model (see Chapter @chap-lin-reg) can be applied to find optimal values of $\boldsymbol{\beta}$ assuming that we have selected a value for the order $m$ of the polynomial fit.

As described in Chapter @chap-lin-reg, an optimal linear model (with optimal $\boldsymbol{\beta}$ values) can be found by minimizing an error function (sum of squared errors) that measures the misfit between the function $y(x, \boldsymbol{\beta})$ and the data points in the considered dataset.

Selecting different values for the order $m$ of the polynomial has a huge impact on the resulting model. In @fig-poly2, we show four examples of the results of fitting polynomials with different orders $m = 0, 1, 3$ and $9$ to our dataset of $n=10$ points.

```{python}
#| echo: false
#| label: fig-poly2
#| fig-cap: 'Plots of polynomials having various orders $M$, shown as green curves, fitted to the considered data set with underlying expected value function in blue.'
#| fig-width: 4
#| fig-height: 4
# NOTE: a degree-9 polynomial needs at least 10 distinct x values.
# We therefore add one additional x very close to the first one (same trick as in the R version).

x2 = np.concatenate([x, [x[0] + 0.001]])
...
for d in degrees:
    ...
    y_hat = model.predict(x_real.reshape(-1, 1))
    frames.append(pd.DataFrame({"x_real": x_real, "variable": f"m_{d}", "value": y_hat}))

melted_df = pd.concat(frames, ignore_index=True)

(
    ggplot(poly_df, aes("x", "y"))
    + geom_point()
    + geom_line(real_df, aes("x_real", "y_real"), color="blue")
    + geom_line(melted_df, aes("x_real", "value"), color="green")
    + facet_wrap("~variable")
    + my_theme
...
def fit_degree9_poly(N, x_pred, seed=13):
    ...
    y_hat = model.predict(x_pred.reshape(-1, 1))
    return xN, yN, y_hat

def build_frames(N, x_pred):
    xN, yN, y_hat = fit_degree9_poly(N, x_pred, seed=13)
    label = f"N = {N}"
    points = pd.DataFrame({"x": xN, "y": yN, "panel": label})
    fit = pd.DataFrame({"x": x_pred, "y": y_hat, "panel": label})
    truth = pd.DataFrame({"x": x_pred, "y": np.sin(2 * np.pi * x_pred), "panel": label})
    return points, fit, truth

points15, fit15, truth15 = build_frames(15, x_real)
points100, fit100, truth100 = build_frames(100, x_real)

points_df = pd.concat([points15, points100], ignore_index=True)
fit_df = pd.concat([fit15, fit100], ignore_index=True)
truth_df = pd.concat([truth15, truth100], ignore_index=True)

(
    ggplot()
    + geom_point(points_df, aes("x", "y"))
    + geom_line(truth_df, aes("x", "y"), color="blue")
    + geom_line(fit_df, aes("x", "y"), color="green")
    + facet_wrap("~panel")
    + my_theme
...
As previously stated, a supervised learning task starts with an available dataset that we use to build a model so that this model will eventually be used in completely independent datasets, as shown in @fig-dataset1.

![](assets/img/topic12_supervised_learning/dataset.png){#fig-dataset1 width=500px}
...
> We assume that the observations $\mathbf{x}_i, i=1...n$ of our dataset and of unseen data are i.i.d., meaning they are independent observations of the same population.

Under this assumption, one common strategy to evaluate the performance of the model on independent datasets is to select a subset of our dataset and pretend it is an independent dataset. As illustrated in @fig-dataset2, we divide the available dataset into a training set and a test set. We will train our algorithm exclusively on the training set and use the test set only for evaluation purposes.

![](assets/img/topic12_supervised_learning/train_test_dataset.png){#fig-dataset2 width=500px}
...
In cross-validation the data is randomly split into a number k of folds (e.g. 3, 5 or 10), as illustrated in @fig-crossval1.

![](assets/img/topic12_supervised_learning/c_val.png){#fig-crossval1 width=700px}
...
sim_base = pd.DataFrame({"x": np.arange(1, 9), "y": [0, 2, 2.5, 5, 7, 6, 8.5, 7]})

simulated_df = pd.concat(
    [
        sim_base.assign(y=sim_base["y"] + rng.normal(0, 0.3, size=len(sim_base)))
        for _ in range(10)
    ],
    ignore_index=True
).sort_values("x")

simulated_df["x"] = simulated_df["x"] + rng.normal(0, 1e-6, size=len(simulated_df))
```
...
#| fig-width: 3
#| fig-height: 3
(ggplot(simulated_df, aes("x", "y")) + geom_point(alpha=0.7) + my_theme)
```
...
#| fig-width: 3
#| fig-height: 3
(ggplot(simulated_df, aes("x", "y")) + geom_point(alpha=0.7) + geom_smooth(span=0.4) + my_theme)
```
...
In Python, we can implement cross-validated models using `scikit-learn`. As an example, we implement a logistic regression model to solve the binary classification task from Chapter @chap-log-reg, which consists in predicting the gender of a person based on height values of him/herself and his/her parents. First, we load the heights dataset:

```{python}
heights_df = pd.read_csv(Path("../../extdata/height.csv")).dropna()
heights_df["sex"] = heights_df["sex"].str.upper()
heights_df = heights_df[heights_df["sex"].isin(["M", "F"])].copy()
heights_df
```

Then, we define the validation specification with `StratifiedKFold`. Here, we set the number of folds `k=5` and we preserve class proportions in each fold.
...
The `cv_results` dictionary returns the ROC-AUC values computed over the $k=5$ models. The performance measurements for each model can be obtained as follows:
...
(
    ggplot(rf_sim, aes("x", "y"))
    + geom_point(color="grey")
    + geom_segment(aes(x=4.5, y=4.5, xend=4.5, yend=238))
    + geom_segment(aes(x=4.5, y=117.5, xend=24, yend=117.5))
    + xlim(1, 24)
    + ylim(1, 238)
    + geom_text(data=region_labels, mapping=aes("x", "y", label="labels"), size=8)
    + my_theme
)
```
...
Additionally, in contrast to regression scenarios, RSS cannot be used as a criterion for making the tree splits for classification tasks. Typically, cross-entropy is used to fit the model (See Section @ML-CE).
...
```{python}
#| echo: false
rng = np.random.default_rng(123458)

n = 10
play_tennis_df = pd.DataFrame({
    "Day": np.arange(1, n + 1),
    "Outlook": rng.choice(["Sunny", "Rainy"], size=n, replace=True),
    "Temperature": rng.choice(np.arange(15, 30.5, 0.5), size=n, replace=True),
    "Humidity": rng.choice(["High", "Normal"], size=n, replace=True),
    "Windy": rng.choice(["Yes", "No"], size=n, replace=True),
})

play_tennis_df["Play.Tennis"] = np.where(
    play_tennis_df["Outlook"] == "Sunny",
    "Yes",
    np.where(play_tennis_df["Humidity"] == "High", "No", "Yes")
)

play_tennis_df.loc[play_tennis_df["Day"] == 1, "Play.Tennis"] = "No"
play_tennis_df.loc[play_tennis_df["Day"] == 1, "Outlook"] = "Sunny"
```
...
![](assets/img/topic12_supervised_learning/rf.png){#fig-rf width=500px}
...
We use `RandomForestClassifier` from `scikit-learn` for training random forests in Python.
...
We remark that the number of trees (`n_estimators`), the minimum size of leaf nodes (`min_samples_leaf`) and the maximum amount of leaf nodes (`max_leaf_nodes`) in each tree are hyperparameters of random forests. These should be optimized (or tuned) for each application to achieve a better performance of the models. Two popular approaches are grid search and random search. Hyperparameter tuning is out the scope of this lecture. We refer the interested reader to the practical methodology chapter of the Deep learning book [@Goodfellow-et-al-2016].
...
```{python}
heights_df["sex_predicted"] = rf_classifier.predict(X)
heights_df
```
...
## Conclusion
By now you should now the concepts of:

**Supervised learning**
* Build and evaluate models that identify trends in the training dataset but are still able
to generalize to (new) independent datasets
* Cross-validation as a strategy to prevent over-fitting
* Cross-validation pitfalls

**Random forests**
* RF fitting for classification and for regression tasks
* Decision trees
* Bagging
* RFs are “random” with respect to features and samples
