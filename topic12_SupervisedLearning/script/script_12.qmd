...
As we can observe, the `cv_results` dictionary returns the ROC-AUC values computed over the $k=5$ models. The performance measurements for each model can be obtained as follows:
...
print(f"Mean ROC-AUC over {k} folds: {roc_mean:.3f} ± {roc_std:.3f}")
```
...
```{python}
log_reg.fit(X, y)
coef = pd.Series(log_reg.coef_.ravel(), index=X.columns).sort_values(key=np.abs, ascending=False)
...
![](assets/img/topic12_supervised_learning/rf_wiki.png){#fig-rf width=500px}
