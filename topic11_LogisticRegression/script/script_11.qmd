...
Instead of modeling the response category directly, we could model the proportion of males per height. The following code does exactly fdfdfthis using 2-cm bins to stratify height:

```{python}
bin_edges = np.arange(
  heights["height"].min() - 1,
  heights["height"].max() + 2,
  2
)
...
heights["mean_height_bins"] = (
  heights
  .groupby("height_bins")["height"]
  .transform("mean")
)
heights["prop"] = (
  heights
  .groupby("height_bins")["y"]
  .transform("mean")
)
...
# Major and minor breaks on log10 scale,
# restricted to the plotted range [0.1, 10]
# -1, 0, 1 -> 0.1, 1, 10
exponents = np.arange(-1, 2, dtype=float)  
breaks = 10.0 ** exponents
minor_raw = (
    np.tile(np.arange(1.0, 10.0), len(exponents))
    * np.repeat(10.0 ** exponents, 9)
)
minor_breaks = minor_raw[
  (minor_raw >= 0.1)
  & (minor_raw <= 10)
]
...
p_log_odds = (
  ...
  + scale_color_manual(
    values={False: "#000000", True: "#999999"}
  )
  + theme(legend_position="none", figure_size=(4, 3))
)
...
\begin{aligned}
\ell(\boldsymbol{\beta})
  &= \sum_i \log p(y_i \mid x_i, \boldsymbol{\beta}) \\[0.5em]
  &= \sum_i \Bigl[
...
The term:
$$
\operatorname{H}(\mathbf{y}, \boldsymbol{\mu}) = - \sum_i \left( y_i \log(\mu_i) + (1-y_i)\log(1- \mu_i) \right)
$$

is called the _cross-entropy_ between the model predictions (the predicted probabilities $\mu_i$) and the observations $y_i$.

Our estimate of $\boldsymbol{\beta}$ shall maximize the likelihood, or equivalently,  minimize the cross-entropy:
$$
\hat{\boldsymbol{\beta}}
  = \arg \max_{\boldsymbol{\beta}} \, \ell(\boldsymbol{\beta})
  = \arg \min_{\boldsymbol{\beta}} \, \operatorname{H}(\mathbf{y}, \boldsymbol{\mu}(\boldsymbol{\beta}))
$$
...
```{python}
prob_5 = logistic_fit.predict(
  pd.DataFrame({"height": [height_p05 + 5]})
)[0]
...
```{python}
prob_10 = logistic_fit.predict(
  pd.DataFrame({"height": [height_p05 + 10]})
)[0]
...
male_sample = male_df.sample(
  n=target_males,
  random_state=123
)

imbalanced_heights = pd.concat(
  [female_df, male_sample],
  ignore_index=True
)
...
```{python}
imbalanced_heights["mu_hat"] = (
  logistic_fit_imbalanced.predict(imbalanced_heights)
)
...
(
  ggplot(
    curve_df,
    aes("height", "mu_hat", color="dataset")
  )
  + geom_line()
  + xlab("Height (cm)")
...
```{python}
heights_balanced["logistic_logit"] = (
  logistic_fit.predict(heights_balanced, linear=True)
)

imbalanced_heights["logistic_logit"] = (
  logistic_fit_imbalanced.predict(imbalanced_heights, linear=True)
)
...
(
  ggplot(
    logit_df,
    aes("height", "logistic_logit", color="dataset")
  )
  + geom_line()
  + xlab("Height (cm)")
...
```{python}
heights["y_multi_pred"] = (
  (multi_logistic_fit.predict(heights) >= 0.5).astype(int)
)
...
```{python}
heights["multi_logistic_mu_hat"] = (
  multi_logistic_fit.predict(heights)
)
roc_rows = []
for label, scores in [
  ("Univariate", heights["mu_hat"]),
  ...
  # to avoid NaNs in ROC computation
  # this is due to some individuals w/o
  # parent height values 
  mask = scores.notna()
  fpr, tpr, _ = roc_curve(
    heights.loc[mask, "y"],
    scores[mask]
...
auc_multi = roc_auc_score(
  heights.loc[mask_multi, "y"],
  heights.loc[mask_multi, "multi_logistic_mu_hat"]
)
...
fpr_imb, tpr_imb, _ = roc_curve(
  imbalanced_heights["y"],
  imbalanced_heights["mu_hat"]
)
auc_imb = roc_auc_score(
  imbalanced_heights["y"],
  imbalanced_heights["mu_hat"]
)

roc_imb_df = pd.DataFrame(
  {"fpr": fpr_imb, "tpr": tpr_imb}
)
...
pr_imb_df = pd.DataFrame(
  {"precision": precision_imb, "recall": recall_imb}
)
...
```{python}
precision_bal, recall_bal, _ = (
  precision_recall_curve(heights["y"],
  heights["mu_hat"]
  )
)

pr_bal_df = pd.DataFrame(
  {"precision": precision_bal, "recall": recall_bal}
)
...
# (quantile-based bins => similar counts per bin)
...
calib_curve_df = pd.DataFrame(
  {"mean_pred": mean_pred, "frac_pos": frac_pos}
)

calib_curve_df = (
  calib_curve_df.dropna()
