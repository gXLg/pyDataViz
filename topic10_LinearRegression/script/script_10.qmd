...
### Maximum likelihood and least squares estimates {#sec-ML-LSE}
...
\begin{aligned}
\hat y
  &= \hat \beta_0 + \hat \beta_1 x, \\[0.5em]
\text{with}\quad
\text{intercept: }\hat \beta_0
  &= \mu_y - \hat \beta_1 \mu_x \\
\text{slope: }\hat \beta_1
...
The Python method `get_prediction` of an `smf.ols` object takes the data as input and returns the prediction. The `summary_frame()` allows extracting the predictions $\hat{y}$ (column `mean`), their standard errors as `mean_se`, and the lower and upper bounds of their 95% confidence intervals as  `mean_ci_lower` and  `mean_ci_upper`:
...
teams_filtered["HR_per_game"] = ( 
    teams_filtered["HR"] / teams_filtered["G"]
)

teams_filtered["R_per_game"] = (
    teams_filtered["R"] / teams_filtered["G"]
)

(
  ggplot(
    teams_filtered,
    aes("HR_per_game", "R_per_game")
...
fitted_model = smf.ols(
  "R_per_game ~ HR_per_game",
  data=teams_filtered).fit()
...
```{python}
#| fig-align: center
teams_filtered["BB_per_game"] = (
  teams_filtered["BB"] / teams_filtered["G"]
)
...
dat = teams_filtered[
  (teams_filtered["HR_strata"] >= 0.4) & (teams_filtered["HR_strata"] <= 1.2)
  ].copy()
```
...
teams_filtered["BB_strata"] = (
  teams_filtered["BB"] / teams_filtered["G"]
).round(1)

dat = teams_filtered[
  (teams_filtered["BB_strata"] >= 2.8) & (teams_filtered["BB_strata"] <= 3.9)
].copy()
...
```{python}
fitted_model = smf.ols(
  "R_per_game ~ HR_per_game + BB_per_game",
  data=teams_filtered).fit()
...
Non-linearity is typically revealed by noticing that the average of the residual depends on the predicted values. A smooth fit (`geom_smooth` default) on the residual plot can help spotting systematic non-linear dependencies (See Figure @fig-non-linear-qc for a simulated example).
