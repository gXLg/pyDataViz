...
This command provided us the airline identifiers present in the dataset. Another valuable information for categorical variables is how often each category occurs. This can be obtained using the following command:
...
```{python}
# this returns an "Index" object
flights.columns 
```

Row index can be customized and can be of any type: integers, strings, dates, tuples etc. Moreover, `pandas` will not control whether your index column
contains only unique or ordered values, but unique, meaningful labels make data easier to work with. Index column can be used for row selection, row
alignment when joining or combining tables, sorting and grouping. 
...
The function `reset_index()` moves the index back into a regular column and the parameter `names` allows giving a custom name to the newly created column:
...
Accessing multiple rows **by index** can be done by creating a vector of indices:
...
If we are now interested in getting all flights that depart from New York area, we can use `isin([list])`:

```{python}
flights[flights["ORIGIN_AIRPORT"].isin(["JFK", "EWR"])]
```

We can also concatenate multiple conditions using the logical OR `|` or the logical AND `&` operator.
...
Using `and` or `or` operators instead, will lead to an error in this case. 

Always wrap mulitple conditions in **parentheses** to make sure, they are processed in the correct order.
...
Missing values in `pandas` are encoded using `np.nan` for floats, `pd.NaT` for datetime columns and `pd.NA` for all other data types. To check whether a value 
...
`sort_values()` function cannot be applied to index column, therefore we have to use `sort_index()`:
...
This function return a new DataFrame by default. Use `inplace=True` to modify the DataFrame directly or assign the returned DataFrame to the same name: 
